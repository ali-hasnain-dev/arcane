...
import { ArcaneSharedProps, NavigationItem, WidgetData } from '../../types';
import { ArrowRight, Users, Activity } from 'lucide-react';

type DashboardPageProps = ArcaneSharedProps & {
...
function DeferredStats() {
    const { stats, arcane } = usePage<DashboardPageProps>().props;
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                ...
                <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">{arcane.navigation.length}</p>
            </div>
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-1">
...
export default function Dashboard() {
    ...
    return (
        <AdminLayout pageTitle="Dashboard">
            ...
            {arcane.navigation.length === 0 ? (
                <div className="text-center py-20 text-zinc-400 dark:text-zinc-500">
                    <p className="text-lg font-medium">No resources registered yet.</p>
                    <p className="text-sm mt-1">
                        Add resources to your <code className="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-xs">ArcaneServiceProvider</code>
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {arcane.navigation.filter((item): item is NavigationItem => item.type === 'item').map(item => (
                        <Link
                            key={item.slug}
                            href={item.url}
                            {...prefetchProps}
                            className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm hover:border-[var(--arcane-primary,#18181b)]/30 dark:hover:border-zinc-600 hover:shadow-md transition-all group"
                        >
                            <div className="flex items-center justify-between">
                                <h3 className="font-semibold text-zinc-800 dark:text-zinc-200">{item.label}</h3>
                                <ArrowRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 group-hover:text-[var(--arcane-primary,#18181b)] dark:group-hover:text-zinc-300 transition-colors" />
                            </div>
